...
- **WebGL (browser)** — With `Lunity.Web.Endpoint` running (e.g. `mix lunity.edit` from a game project), open **`/player`** for a minimal parity shell (`priv/static/player.html` + `/static/player_shell.js`). A game may override by placing `priv/static/player.html` in its own app (resolved via `Lunity.project_app()`). Same bootstrap as `mix lunity.player`: query `token` + `jwt`, or `token` + `mint_key` + `user_id`, optional `hints` JSON and `auth_only=1`. WebSockets use **`check_origin: :conn`** so opening the page at **`127.0.0.1`** still upgrades when the default endpoint URL host would be **`localhost`** (avoids a silent 403 on `/ws/player`).
