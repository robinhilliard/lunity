...
4. **`auth`** — `{ "token": "<JWT>" }`, optionally `{ "resume": true }` to reattach the previous input session after a disconnect (same JWT `user_id` / `player_id` claims; **JWT-only** — no extra ticket). Only works within **`:player_reconnect_grace_ms`** after the prior socket closed (default **10s**). → **`ack`** with `user_id` and `player_id`, or **`ack`** with **`resumed`: true** when state was restored, or **`error`**. JWTs are HS256 and validated by [`Lunity.Auth.PlayerJWT`](lib/lunity/auth/player_jwt.ex) (claims `user_id`, optional `player_id`).
   - **Resume `ack` (in-world):** when the restored session had an **`instance_id`**, **`ack`** also includes **`instance_id`**, **`entity_id`**, and **`spawn`** (same meaning as in **`assigned`**). **`mix lunity.player`**, **`/player`**, and **`Lunity.Player.WsClient`** then **skip `join`** and continue with **`subscribe_state`** when follow-up is enabled. You still **must** send **`subscribe_state`** on each new connection — periodic **`state`** pushes are tied to the socket process and are **not** auto-resumed.
5. **`join`** → **`assigned`** or **`error`** (skip if step 4 already returned **`instance_id`** in a resume **`ack`**).
...
| `:player_reconnect_grace_ms` | After a disconnect, keep the ETS input session this long for **`auth`** + **`resume`: true** (default `10000`). |
